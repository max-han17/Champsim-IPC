...
int main(int argc, char** argv) { 
    ...
    for (int i = 0; i < iters; i++) {
        if (rank == RANK0) {
            ...
            task->done_ = false;
            TRACE_ACCESS(rank, "WRITE task->done_", &task->done_);
            queue->emplace(1);
            TRACE_ACCESS(rank, "WRITE queue->emplace", queue);
            while (!task->done_) {
                TRACE_ACCESS(rank, "READ task->done_", &task->done_);
                continue;
            }
        } else {
            ...
            while (queue->pop(x).IsNull()) {
                TRACE_ACCESS(rank, "READ queue->pop", queue);
                continue;
            }
            task->done_ = true;
            TRACE_ACCESS(rank, "WRITE task->done_", &task->done_);
