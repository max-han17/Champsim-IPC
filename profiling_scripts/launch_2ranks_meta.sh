...
if [ "$RANK" -eq 0 ]; then
    exec $PIN_BIN -t $TRACER_SO -o $TRACE_DIR/meta_rank0_20k.champsim -s 0 -t $TRACE_INSTR -- $APP
else
    exec $PIN_BIN -t $TRACER_SO -o $TRACE_DIR/meta_rank1_20k.champsim -s 0 -t $TRACE_INSTR -- $APP
fi
